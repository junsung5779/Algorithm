...
for i in range(T):
    ...
    if len(word) >= 6:
        ...
        queue.append(word[-(len(word)-5)])
    # pop인 경우
    elif word == 'pop':
        if queue:  # 큐에 정수가 들어있는 경우
            print(queue.pop(0)) #큐에서 가장 앞에 있는 정수를 빼고, 그 수를 출력
            # 리스트로 선언해서 pop(0)를 하게 되면, 첫 번째 요소를 pop 하고나서 나머지 요소들의 인덱스를 1칸씩 당기는 과정에서 O(n)의 계산량이 발생하기 때문에 시관초과
        else:  # 큐에 들어가있는 정수가 없는 경우 -1을 출력
            print(-1)
    # size인 경우
    elif word == 'size':
    ...
    # front인 경우
        ...
        if queue:  # 큐에 정수가 들어있는 경우
            print(queue[0])     #큐의 가장 앞에 있는 정수를 출력
        else:  # 큐에 들어가있는 정수가 없는 경우 -1을 출력
            print(-1)
    # back인 경우
    elif word == 'back':
