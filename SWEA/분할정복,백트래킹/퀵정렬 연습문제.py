...
def hoare_partition(arr, start, end):
    ...
    i = start   # 오른쪽으로 한 칸씩 이동하면서, pivot보다 큰값의 인덱스에서 정지
    j = end     # 왼쪽으로 한 칸씩 이동하면서, pivot보다 작은값의 인덱스에서 정지
    # pivot과 큰 값과, 작은 값으로 영역 구분
    while i <= j:
        # i가 j보다 같거나 작을때만 i는 증가하면서 pivot보다 큰 값 찾기
